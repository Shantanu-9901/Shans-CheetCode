...
export class FeedbackView extends LitElement {
    ...
    async _loadVersion() {
        try {
            this._version = await cheetCode.getVersion();
            this.requestUpdate();
        } catch (e) {}
    }
    ...
    async _submitFeedback() {
        ...
        try {
            ...
            // Feedback endpoint not configured yet
            this._feedbackStatus = 'error:Feedback is not available yet';
            this._feedbackSending = false;
            this.requestUpdate();
            return;

            const res = await fetch('https://localhost/api/feedback', {
                method: 'POST',
