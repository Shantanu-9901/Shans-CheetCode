...
export class CustomizeView extends LitElement {
    ...
    getThemes() {
        return cheetCode.theme.getAll();
    }

    async _loadFromStorage() {
        try {
            const [prefs, keybinds] = await Promise.all([cheetCode.storage.getPreferences(), cheetCode.storage.getKeybinds()]);
    ...
    getDefaultKeybinds() {
        const isMac = cheetCode.isMacOS || navigator.platform.includes('Mac');
        return {
    ...
    async saveKeybinds() {
        await cheetCode.storage.setKeybinds(this.keybinds);
        if (window.require) {
    ...
    async handleCustomPromptInput(e) {
        ...
        await cheetCode.storage.updatePreference('customPrompt', this.customPrompt);
    }

    async handleAudioModeSelect(e) {
        this.audioMode = e.target.value;
        await cheetCode.storage.updatePreference('audioMode', this.audioMode);
        this.requestUpdate();
    }

    async handleThemeChange(e) {
        this.theme = e.target.value;
        await cheetCode.theme.save(this.theme);
        this.updateBackgroundAppearance();
        this.requestUpdate();
    }
    ...
    async handleGoogleSearchChange(e) {
        ...
        await cheetCode.storage.updatePreference('googleSearchEnabled', this.googleSearchEnabled);
        if (window.require) {
            try {
                const { ipcRenderer } = window.require('electron');
    ...
    async handleBackgroundTransparencyChange(e) {
        ...
        await cheetCode.storage.updatePreference('backgroundTransparency', this.backgroundTransparency);
        this.updateBackgroundAppearance();
        this.requestUpdate();
    }

    updateBackgroundAppearance() {
        const colors = cheetCode.theme.get(this.theme);
        cheetCode.theme.applyBackgrounds(colors.background, this.backgroundTransparency);
    }

    async handleFontSizeChange(e) {
        this.fontSize = parseInt(e.target.value, 10);
        await cheetCode.storage.updatePreference('fontSize', this.fontSize);
        this.updateFontSize();
        this.requestUpdate();
    }
    ...
    async resetKeybinds() {
        this.keybinds = this.getDefaultKeybinds();
        await cheetCode.storage.setKeybinds(null);
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            ipcRenderer.send('update-keybinds', this.keybinds);
        }
    ...
    async restoreAllSettings() {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(defaults)) {
                await cheetCode.storage.updatePreference(key, value);
            }
            ...
            this.updateBackgroundAppearance();
            this.updateFontSize();
            await cheetCode.theme.save(defaults.theme);
    ...
    async clearLocalData() {
        ...
        this.requestUpdate();
        try {
            await cheetCode.storage.clearAll();
