...
export class MainView extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            const [prefs, creds] = await Promise.all([cheetCode.storage.getPreferences(), cheetCode.storage.getCredentials().catch(() => ({}))]);

            this._mode = prefs.providerMode === 'cloud' ? 'byok' : prefs.providerMode || 'byok';

            // Load keys
            this._token = creds.cloudToken || '';
            this._geminiKey = (await cheetCode.storage.getApiKey().catch(() => '')) || '';
            this._groqKey = (await cheetCode.storage.getGroqApiKey().catch(() => '')) || '';
    ...
    async _saveMode(mode) {
        ...
        this._tokenError = false;
        this._keyError = false;
        await cheetCode.storage.updatePreference('providerMode', mode);
        this.requestUpdate();
    }
    ...
    async _saveToken(val) {
        ...
        this._tokenError = false;
        try {
            const creds = await cheetCode.storage.getCredentials().catch(() => ({}));
            await cheetCode.storage.setCredentials({ ...creds, cloudToken: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveGeminiKey(val) {
        this._geminiKey = val;
        this._keyError = false;
        await cheetCode.storage.setApiKey(val);
        this.requestUpdate();
    }

    async _saveGroqKey(val) {
        this._groqKey = val;
        await cheetCode.storage.setGroqApiKey(val);
        this.requestUpdate();
    }
    ...
    async _saveOpenaiKey(val) {
        ...
        try {
            const creds = await cheetCode.storage.getCredentials().catch(() => ({}));
            await cheetCode.storage.setCredentials({ ...creds, openaiKey: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveOllamaHost(val) {
        ...
        await cheetCode.storage.updatePreference('ollamaHost', val);
        this.requestUpdate();
    }

    async _saveOllamaModel(val) {
        this._ollamaModel = val;
        await cheetCode.storage.updatePreference('ollamaModel', val);
        this.requestUpdate();
    }

    async _saveWhisperModel(val) {
        this._whisperModel = val;
        await cheetCode.storage.updatePreference('whisperModel', val);
        this.requestUpdate();
    }
        ...
        return html`
            <div class="form-wrapper">
                ${this._mode === 'local'
                    ? html`
                          <div class="title-row">
                              <div class="page-title">Shan's CheetCode <span class="mode-suffix">Local AI</span></div>
                              <button
                                  class="help-btn"
                                  @click=${() => {
                      ...
                      `
                    : html` <div class="page-title">${html`Shan's CheetCode <span class="mode-suffix">BYOK</span>`}</div> `}
