...
export class CheetCodeApp extends LitElement {
    static styles = css`
        * {
    ...
    async _checkForUpdates() {
        try {
            this._localVersion = await cheetCode.getVersion();
            this.requestUpdate();
        } catch (e) {
            // silently ignore
        }
    ...
    async _loadFromStorage() {
        try {
            const [config, prefs] = await Promise.all([cheetCode.storage.getConfig(), cheetCode.storage.getPreferences()]);
    ...
    async handleStart() {
        const prefs = await cheetCode.storage.getPreferences();
        const providerMode = prefs.providerMode || 'byok';

        if (providerMode === 'local') {
            const success = await cheetCode.initializeLocal(this.selectedProfile);
            if (!success) {
            ...
            const apiKey = await cheetCode.storage.getApiKey();
            if (!apiKey || apiKey === '') {
                const mainView = this.shadowRoot.querySelector('main-view');
                if (mainView && mainView.triggerApiKeyError) {
                    mainView.triggerApiKeyError();
            ...
            await cheetCode.initializeGemini(this.selectedProfile, this.selectedLanguage);
        }

        cheetCode.startCapture(this.selectedScreenshotInterval, this.selectedImageQuality);
        this.responses = [];
        this.currentResponseIndex = -1;
    ...
    async handleAPIKeyHelp() {
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            await ipcRenderer.invoke('open-external', 'https://github.com/shantanu');
        }
    }
    ...
    async handleProfileChange(profile) {
        ...
        await cheetCode.storage.updatePreference('selectedProfile', profile);
    }

    async handleLanguageChange(language) {
        this.selectedLanguage = language;
        await cheetCode.storage.updatePreference('selectedLanguage', language);
    }

    async handleScreenshotIntervalChange(interval) {
        this.selectedScreenshotInterval = interval;
        await cheetCode.storage.updatePreference('selectedScreenshotInterval', interval);
    }

    async handleImageQualityChange(quality) {
        this.selectedImageQuality = quality;
        await cheetCode.storage.updatePreference('selectedImageQuality', quality);
    }

    async handleLayoutModeChange(layoutMode) {
        this.layoutMode = layoutMode;
        await cheetCode.storage.updateConfig('layout', layoutMode);
        if (window.require) {
            try {
                const { ipcRenderer } = window.require('electron');
    ...
    async handleSendText(message) {
        const result = await window.cheetCode.sendTextMessage(message);
        if (!result.success) {
            ...
            <div class="sidebar ${this._isLiveMode() ? 'hidden' : ''}">
                ...
                <div class="sidebar-footer">
                    ${this._updateAvailable
                        ? html`
                              <button class="update-btn" @click=${() => this.handleExternalLinkClick('https://github.com/shantanu')}>
...
customElements.define('cheetcode-app', CheetCodeApp);
