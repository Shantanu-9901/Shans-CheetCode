...
async function initializeGemini(profile = 'interview', language = 'en-US') {
    ...
    if (apiKey) {
        ...
        if (success) {
            cheetCode.setStatus('Live');
        } else {
            cheetCode.setStatus('error');
        }
    }
}
...
async function initializeLocal(profile = 'interview') {
    ...
    if (success) {
        cheetCode.setStatus('Local AI Live');
        return true;
    } else {
        cheetCode.setStatus('error');
        return false;
        ...
        async blob => {
            ...
            reader.onloadend = async () => {
                ...
                if (result.success) {
                    ...
                    cheetCode.addNewResponse(`Error: ${result.error}`);
                }
            };
            reader.readAsDataURL(blob);
        },
...
function handleShortcut(shortcutKey) {
    const currentView = cheetCode.getCurrentView();

    if (shortcutKey === 'ctrl+enter' || shortcutKey === 'cmd+enter') {
        if (currentView === 'main') {
            cheetCode.element().handleStart();
        } else {
...
const cheetCodeApp = document.querySelector('cheetcode-app');
...
// Consolidated cheetCode object - all functions in one place
const cheetCode = {
    // App version
    getVersion: async () => ipcRenderer.invoke('get-app-version'),

    // Element access
    element: () => cheetCodeApp,
    e: () => cheetCodeApp,

    // App state functions - access properties directly from the app element
    getCurrentView: () => cheetCodeApp.currentView,
    getLayoutMode: () => cheetCodeApp.layoutMode,

    // Status and response functions
    setStatus: text => cheetCodeApp.setStatus(text),
    addNewResponse: response => cheetCodeApp.addNewResponse(response),
    updateCurrentResponse: response => cheetCodeApp.updateCurrentResponse(response),
