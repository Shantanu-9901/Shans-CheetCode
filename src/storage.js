...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'shans-cheetcode-config');
    } else if (platform === 'darwin') {
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'shans-cheetcode-config');
    } else {
        configDir = path.join(os.homedir(), '.config', 'shans-cheetcode-config');
    }

    // Migrate from old config directory if it exists
    const oldName = configDir.replace('shans-cheetcode-config', 'cheating-daddy-config');
    if (!fs.existsSync(configDir) && fs.existsSync(oldName)) {
        try {
            fs.cpSync(oldName, configDir, { recursive: true });
            console.log('Migrated config from', oldName, 'to', configDir);
        } catch (e) {
